...
def test_csv_a(args_csv_a, data_csv_a):
  ...
  #     print(f'{index_name}   <{target_name}>   sv: {src_value}', end = '')
  ...
  for target_file in resultant_files:
    ...
    for row in target_data.itertuples(index = True):
      ...
      for i, col in enumerate(target_data.columns.values):
        ...
        src_value = src_data.loc[[index_name], [col]].values[0,0]

        print(f'{target_processed_count + 1}) {index_name}   <{target_name}>   {col}   tv: {target_value}    sv: {src_value}')
        if math.isclose(target_value, src_value, rel_tol=1e-20) == False:
          pprint(src_data)
          sys.exit(0)
